...
# Chances of running out: 1 - (chances of using 0-7)
today <- 1 - ppois(7,lambda=dist$estimate)

# Or we can use the lower.tail parameter instead of subtracting from 1
today <- ppois(7,lambda=dist$estimate, lower.tail = FALSE)

# Chances of running out tomorrow: 1 - (chances of using 0-4) with the new Poisson
tomorrow <- 1 - ppois(4,lambda=(2*dist$estimate))

# Generalized for any n day, currently set to 3, with 10 on hand
any.n <- 3
onhand <- 10
anyday <- 1- ppois((onhand - 3*any.n),lambda=(any.n*dist$estimate))
